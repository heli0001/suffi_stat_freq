...
###############
library(lme4)
logran = glmer(A ~ 1 + x1 + x2+ (1|clu),data=data,family=binomial,nAGQ=10)
...
library(survival)
clog_summ = clogit(A~x1+x2+strata(clu), data=data) ## multiple times and the mean should be close to 1 for each;
